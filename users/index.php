...
if (isset($_POST['submit'])) {	
	...
	$ret = mysqli_query($bd, "SELECT * FROM users WHERE matricNo= '$matric' and password='$password' ");
	$num = mysqli_fetch_array($ret);
	if ($num > 0) {
		$extra = "dashboard.php"; //
		$userid = $num['id'];	
		$_SESSION['login'] = $matric;
		$_SESSION['id'] = $userid ;		
		$status = 1;
		$qry = "insert into userlog(uid,username,matricNo,userip,loginTime,logout,status) values('$userid', '', '$matric', '$uip', NOW(), '', '$status' )";
		$log = mysqli_query($bd, $qry);		
		...
		$status = 0;
		$userid = 0;
		mysqli_query($bd, "insert into userlog(uid,username,matricNo,userip,loginTime,logout,status) values('$userid', '', '$matric', '$uip', NOW(), '', '$status' )");
		$errormsg = "Invalid Matric no or password";
		$extra = "login.php";
	}
}
...
if (isset($_POST['change'])) {
	...
	$matric = $_POST['matric'];
	$password = md5($_POST['password']);
	$query = mysqli_query($bd, "SELECT * FROM users WHERE userEmail='$email' and matricNo='$matric'");
	$num = mysqli_fetch_array($query);
	if ($num > 0) {
		mysqli_query($bd, "update users set password='$password' WHERE userEmail='$email' and matricNo='$matric' ");
...
<head>
	...
	<title>iReporting System | User Login</title>
	...
	<div id="login-page">
		<div class="container">
			<form class="form-login" name="login" method="post">
				<div style="display:flex; background: #a80a0a; padding: 20px;">
					<img src="../img/unilag_logo.png">
					<div style="align-self: center;">
						<h2 class="form-headings">Student iReporting System</h2>
					</div>
				</div>
				...
				<div class="login-wrap">
					...
					<input type="number" class="form-control" name="matricNo" placeholder="Matric No" required autofocus>
					<br>
					<input type="password" class="form-control" name="password" required placeholder="Password">
					<label class="checkbox">
						<span class="pull-right">
							<a data-toggle="modal" href="login.html#forgotPasswordModal"> Forgot Password?</a>
						</span>
					</label>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<input type="text" name="matric" placeholder="matric No" autocomplete="off" class="form-control" required><br>

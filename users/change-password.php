...
if (strlen($_SESSION['login']) == 0) {
  ...
  if (isset($_POST['submit'])) {
    $sql = mysqli_query($bd, "SELECT password FROM  users where password='" . md5($_POST['password']) . "' && matricNo='" . $_SESSION['login'] . "'");
    $num = mysqli_fetch_array($sql);
    if ($num > 0) {
      $con = mysqli_query($bd, "update users set password='" . md5($_POST['newpassword']) . "', updationDate='$currentTime' where matricNo='" . $_SESSION['login'] . "'");
      if($con){
        $successmsg = "Password Changed Successfully !!";
      } else {
      $errormsg = 'Error: Password not changed';
      }
    } else {
      $errormsg = "Old Password did not match !!";
    }
  }
?>
